...
class SpongeBobConfig(PretrainedConfig):
    ...
    def __init__(
            ...
            num_hidden_layers: int = 16, # Transformer block 的层数
            num_attention_heads: int = 16, # 注意力头的数量(Query头数)
            num_key_value_heads: int = 8, # KV 头数（用于 Grouped Query Attention）
            intermediate_size: int = 3072,##FFN中间维度

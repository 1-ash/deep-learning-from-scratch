...
from transformers.activations import ACT2FN # 激活函数，比torch实现的更加全面
...
class Attention(nn.Module):
    ...
    def __init__(self, args: SpongeBobConfig):
        ...
        # 表示先检查你当前 PyTorch 版本里有没有torch.nn.functional.scaled_dot_product_attention这个函数
        self.flash = hasattr(torch.nn.functional, 'scaled_dot_product_attention') and args.flash_attn

    def forward(self,
                x: torch.Tensor,
                position_embeddings: Tuple[torch.Tensor, torch.Tensor],
                past_key_value: Optional[Tuple[torch.Tensor, torch.Tensor]] = None, # Optional[T] = T 或 None
                use_cache=False,
        ...
        # KV cache 实现（用于推理加速）
        # xk:[batch_size,seq_len,num_kv_heads,head_dim] dim=1 是序列长度维度
        ...
        # 使用 Flash Attention - 仅在训练时且无 KV cache 时启用
...
class SpongeBobBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: SpongeBobConfig):
        ...
        # 因为RMSNorm是可学习的层，且不同层数据分布不同，所以必须一个层单独设置一个
...
class SpongeBobForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    def forward(self,
        ...
        if labels is not None:
            ...
            shift_logits = logits[..., :-1, :].contiguous() # shift_logits : (batch_size, seq_len-1, vocab_size)

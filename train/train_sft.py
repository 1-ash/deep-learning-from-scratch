...
from dataset.sft_data.sft_dataset import SFTDataset  # [SFT] pretrain 用 PretrainDataset(.bin)，SFT 用 SFTDataset(jsonl + 只算 assistant loss)
...
def train_epoch(epoch, loader, iters, start_step=0, swanlab=None, total_steps=None, warmup_steps=None, full_save_dir=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if args.enable_eval and getattr(args, "eval_interval", 0) > 0 and global_step % args.eval_interval == 0 and is_main_process():
            from benchmark.minibench_sft.eval import run_inference, run_judge_async
...
if __name__ == "__main__":
    ...
    parser.add_argument("--judge_api_key", type=str, default='sk-7bd350df31da4294957b62e0061a1211', help="Judge API Key（可直接传入或从环境变量 DEEPSEEK_API_KEY 读取）")

...
def train_epoch(epoch, loader, iters, start_step=0, swanlab=None, total_steps=None, warmup_steps=None, full_save_dir=None):
    ...
    Args:
        ...
        iters：这个 epoch 总共有多少步
        start_step=0：从哪个 step 开始，通常断点续训会用到
        swanlab：实验记录工具，用来记 loss、lr 等
        total_steps：训练总步数，用于学习率调度
        warmup_steps：warmup 的步数
        full_save_dir：checkpoint 保存目录
    """
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        # with autocast_ctx开启自动混合精度（AMP
        with autocast_ctx:
            res = model(input_ids, labels=labels)
            loss = res.loss / args.accumulation_steps
        # 使用梯度缩放器（scaler）来缩放梯度，避免梯度爆炸
        ...
        if step % args.log_interval == 0 or step == iters - 1:
            ...
            if swanlab: # swanlab.log 记录损失、学习率、时间等信息
        ...
        if args.eval_bench == 1 and tokenizer is not None and global_step % args.eval_interval == 0:
            ...
            xcopa_path = './benchmark/clue_xcopa_eval_500.jsonl'
...
if __name__ == "__main__":
    ...
    ckp_data = None
    if args.from_resume == 1: # 从 checkpoint 恢复训练
    ...
    if args.use_swanlab:
        import swanlab
        swanlab.login(api_key="pWSSR0qR01AUw2IeRUteN")
